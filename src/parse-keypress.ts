...
export function parseMultipleKeypresses(prevState: KeyParseState, input: Buffer | string | null = ''): [ParsedKey[], KeyParseState] {
  ...
  while(text) { matchers[state.mode](); }
  return [keys, state]
}
...
export type ParsedKey = {
	...
	code?: string;
	isPasted: boolean; // Indicates this key came from pasted content
};

const parseKeypress = (s: string = ''): ParsedKey => {
	let parts;
